# DELTARUNE Runtime and Modding Reference

DELTARUNE uses a separate launcher and separate runtime payload for each of
Chapters 1–5. Each chapter owns its rooms, battle logic, save/load behavior,
localization helpers, and chapter-specific systems. The launcher reads shared
configuration and save metadata, then hands control to the selected chapter.

This reference explains shipped runtime behavior, chapter differences, content
status, and practical modification points. A definition being present does not
by itself prove that normal gameplay reaches it. Status terms for conditional,
debug, legacy, removed, and unresolved content are defined in
[Debug, Legacy, and Unused Content](debug-legacy-and-unused-content.md).

## Runtime and platform

- [Runtime Architecture](runtime-architecture.md)
- [Chapter Select](chapter-select.md)
- [Chapter Differences](chapter-differences.md)
- [Platform Differences](platform-differences.md)
- [Global Variables](global-variables.md)
- [Save and Load](save-load-system.md)
- [Localization](localization.md)

## World, events, and dialogue

- [Overworld](overworld.md)
- [Rooms, Events, and Progression](rooms-events-and-progression.md)
- [Cutscene System](cutscene-system.md)
- [Dialogue System](dialogue-system.md)
- [Speaker System](speaker-system.md)
- [Funnytext System](funnytext-system.md)
- [Game Over System](game-over-system.md)

## Battle

- [Battle System](battle-system.md)
- [Enemy, Encounter, and Attack Catalog](enemies-encounters-and-attacks.md)
- [Battle Catalog Data](battle-catalog-data.md)
- [Monster Runtime](monster-runtime.md)
- [Damage System](damage-system.md)
- [Tension and TP](tension-system.md)
- [Heart and SOUL Mechanics](heart-mechanics.md)
- [Recruit System](recruit-system.md)

## Characters, abilities, and economy

- [Characters, Stats, and Progression](characters-stats-and-progression.md)
- [Party Management](party-management.md)
- [Level-Up System](level-up-system.md)
- [Items and Equipment](items-equipment.md)
- [Spells and Abilities](spells-abilities.md)
- [Shops and Economy](shops-and-economy.md)
- [Fusion System](fusion-system.md)

## Chapter-specific and presentation systems

- [Board System](board-system.md)
- [Rhythm System](rhythm-system.md)
- [Chapter 5 Systems](chapter-5-systems.md)
- [Rendering, Animation, and Audio](rendering-animation-audio.md)

## Content status

- [Debug, Legacy, and Unused Content](debug-legacy-and-unused-content.md)
